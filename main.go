...
import (
	...
	"github.com/anthropics/anthropic-sdk-go" // need 1.4 because 1.5 changes defn of anthropic.NewToolResultBlock
...
// needed to "generate a JSON schema for our tool definition which we send to the model"
...
type EditFileInput struct {
	...
	OldStr string `json:"old_str" jsonschema_description:"Text to be replaced, to search for - must match exactly and must have only one match, exactly`
	NewStr string `json:'new_str" jsonschema_description:"Text that will replace old_str"`
}
...
func EditFile(input json.RawMessage) (string, error) {
	...
	if editFileInput.Path == "" || editFileInput.OldStr == editFileInput.NewStr {
		return "", fmt.Errorf("invalid input parameter(s)")
	}
